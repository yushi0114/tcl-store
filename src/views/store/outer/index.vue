...
<script lang="ts">
  ...
  import OuterDetailDrawer from './OuterDetailDrawer.vue';

  export default defineComponent({
    name: 'DeptManagement',
    components: { BasicTable, DeptModal, TableAction, PageWrapper, OuterDetailDrawer },
    setup() {
      ...
      const { run: ExportOuterOrder } = useRequest(exportOuterOrder, {
        onSuccess(data) {
          downloadByData(data, '出库单.xlsx');
        },
      });
      const { run: DeleteIncomingOrder } = useRequest(deleteOuterOrder, {
        onSuccess() {
          reload();
        },
        onError(error) {
      ...
      const [registerTable, { reload, setLoading, getForm, getDataSource }] = useTable({
        title: '出库列表',
        api: getOuterOrderPage,
        columns,
        formConfig: {
          labelWidth: 120,
      ...
      function handleExport() {
        ...
        ExportOuterOrder(getFieldsValue() as any);
      }
